...
\begin{atentie}
\textbf{Autentificare prin parola.} GitHub a renuntat la autentificarea prin parola pentru operatiunile git (din 2021). Solutia recomandata este generarea unui \emph{Personal Access Token} (PAT) la \url{https://github.com/settings/tokens} cu scope-ul \code{repo}, folosit ulterior in locul parolei. Tokenul este memorat de \emph{Git Credential Manager} (Windows) sau \emph{Keychain} (macOS), nu de browser \textendash{} la urmatorul \code{git push} nu va mai fi cerut.
\end{atentie}
...
\begin{enumerate}[itemsep=2pt]
  ...
  \item Pentru un repository numit \code{portfolio} al utilizatorului \code{voluntar-best}, care este URL-ul GitHub Pages rezultat?
\end{enumerate}
\end{selfcheck}
