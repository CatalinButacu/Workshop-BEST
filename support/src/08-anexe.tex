...
\begin{description}[font=\sffamily\bfseries\color{partcolor}, leftmargin=0pt, labelindent=0pt, style=nextline, itemsep=4pt]
  ...
  \item[Cubic-bezier] Func\textcommabelow{t}ie de timing CSS care define\textcommabelow{s}te o curba de accelerare prin patru numere (doua puncte de control), pentru anima\textcommabelow{t}ii personalizate.
  \item[Debounce] Tehnica JS care amana executia unei func\textcommabelow{t}ii pana cand evenimentul nu se mai declan\textcommabelow{s}eaza \emph{N} ms. Folosit pentru input/autocomplete.
  ...
  \item[Font-display] Proprietate \code{@font-face} care controleaza ce vede utilizatorul cat timp fontul se descarca. \code{swap} = arata fallback imediat, evita FOIT.
  ...
  \item[IntersectionObserver] API browser care notifica un callback cand un element intra/iese din viewport. Inlocuieste handlerele de scroll pentru anima\textcommabelow{t}ii \textcommabelow{s}i lazy loading.
  ...
  \item[Preconnect] Hint HTML (\code{<link rel="preconnect">}) care instruieste browserul sa stabileasca conexiune (DNS + TCP + TLS) cu un domeniu in avans, reducand latenta pentru prima cerere.
  ...
  \item[\texttt{requestAnimationFrame}] API browser care apeleaza un callback inainte de fiecare repaint (\textasciitilde 60 fps). Inlocuieste \code{setTimeout} pentru anima\textcommabelow{t}ii JavaScript fluide.
  \item[Responsive design] Abordare prin care site-ul se adapteaza dimensiunii ecranului utilizatorului.
  \item[\texttt{robots.txt}] Fisier text in radacina site-ului care indica robotilor de crawl ce pagini sa indexeze \textcommabelow{s}i unde se gase\textcommabelow{s}te sitemap-ul.
  ...
  \item[Service Worker] Script care ruleaza in fundalul browserului, poate intercepta cererile HTTP. Baza pentru PWA \textcommabelow{s}i func\textcommabelow{t}ionalitate offline.
  \item[\texttt{sitemap.xml}] Fisier XML care listeaza URL-urile importante ale site-ului. Submis in Google Search Console pentru indexare mai rapida.
  ...
  \item[Throttle] Tehnica JS care permite executia unei func\textcommabelow{t}ii cel mult o data la \emph{N} ms. Folosit pentru \code{scroll}, \code{resize}.
...
\begin{itemize}[leftmargin=*, label={$\square$}, itemsep=2pt]
  ...
  \item \code{<link rel="canonical">} catre URL-ul oficial.
  \item \code{robots.txt} prezent in radacina, cu directiva \code{Sitemap:}.
  \item \code{sitemap.xml} prezent in radacina; trimis in Google Search Console.
...
\begin{itemize}[leftmargin=*, label={$\square$}, itemsep=2pt]
  ...
  \item Imagini convertite in WebP / AVIF, redimensionate la latimea de afi\textcommabelow{s}are.
  \item \code{<img>} are \code{width}, \code{height}, \code{loading="lazy"} (excep\textcommabelow{t}ie: hero).
  \item Fonturi cu \code{font-display: swap}; fontul critic e \code{preload}-at.
